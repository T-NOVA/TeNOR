...
WORKDIR /root/TeNOR
RUN gem install bundle && bundle install && ./tenor_install.sh
...
COPY install_gatekeeper.sh /root/install_gatekeeper.sh
RUN sh install_gatekeeper.sh

RUN cp /root/go/src/github.com/piyush82/auth-utils/gatekeeper.cfg /root/go/bin/gatekeeper.cfg
