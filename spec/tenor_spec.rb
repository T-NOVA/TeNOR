			...
			it 'create pops' do
				@pops_names.each do |pop_name|
					...
					pop = {
				      ...
				      is_admin: is_admin,
				      description: "",
				      extra_info: "keystone-endpoint=http://#{host}:35357/#{keystone_version} orch-endpoint=http://#{host}:8004/v1 compute-endpoint=http://#{host}:8774/v2.1 neutron-endpoint=http://#{host}:9696/v2.0 dns=#{dns}"
				  }
