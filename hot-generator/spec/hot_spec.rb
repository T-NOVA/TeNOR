...
RSpec.describe HotGenerator do
  ...
  describe 'POST /hot/:flavour' do
    context 'given an invalid content type' do
      let(:response) { post '/hot/flavor0', {vnfd: {}, networks_id: 'network_id', security_group_id: "security_group_id3", vnfr_id: "vnfr_id"}.to_json, rack_env={'CONTENT_TYPE' => 'application/x-www-form-urlencoded'} }
    ...
    context 'given an invalid content type' do
      let(:response) { post '/hot/flavor0', {vnfd: {}, networks_id: 'network_id'}.to_json, rack_env={'CONTENT_TYPE' => 'application/json'} }
    ...
    context 'given a valid VNFD' do
      ...
      networks_id = []
      instance_info = {:vnf => JSON.parse(vnfd), :networks_id => networks_id, :security_group_id => "security_group_id2", :routers_id => "routers_id", :vnfr_id => "vnfr_id", :dns_server=> "dns_server", :public_network_id=> "network", :flavours => []}
