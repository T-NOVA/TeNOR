...
class OrchestratorNsMonitoring < Sinatra::Application
  ...
  get '/ns-monitoring/:instance_id/monitoring-data/' do
    ...
    if params[:metric] && !params[:start]
      ...
      @db.execute("SELECT metricName, date, unit, value FROM nsmonitoring WHERE instanceid='#{params[:instance_id].to_s}' AND metricname='#{params[:metric].to_s}' AND date >= #{params[:start]} LIMIT 2000 ").fetch { |row| t.push(row.to_hash) }
    elsif params[:metric] && params[:start] && params[:end]
